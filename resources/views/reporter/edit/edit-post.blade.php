    ...
    <div class="p-4">
        <div class="modal-content">
            <div class="modal-header">
                <h2 style="font-size:20px; padding-bottom:5px">Edit Posts</h2>
            </div>
            <div class="modal-body pt-2">
                <form action="{{ route('reporter.update.post') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="postTitle"
                                name="title" placeholder="Enter post title" value="{{ old('title', $posts->title) }}">
                            @error('title')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('imgTitle') is-invalid @enderror"
                                id="imgTitle" name="imgTitle" placeholder="Enter img title"
                                value="{{ old('imgTitle', $posts->imgTitle) }}">
                            @error('imgTitle')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('reporter') is-invalid @enderror"
                                ...
                                value="{{ old('reporter', $posts->reporter) }}">
                            @error('reporter')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('subtitle') is-invalid @enderror"
                                ...
                                value="{{ old('subtitle', $posts->subtitle) }}">
                            @error('subtitle')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                            ...
                            <select class="form-select @error('subcategoryId') is-invalid @enderror" id="postSubCategory"
                                ...
                                @foreach ($sub_categories as $sub_category)
                                    <option value="{{ $sub_category->id }}"
                                        {{ $sub_category->id == $posts->subcategory_id ? 'selected' : '' }}>
                                        {{ $sub_category->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select @error('districtsId') is-invalid @enderror" id="postDistrict"
                                ...
                                @foreach ($districts as $district)
                                    <option value="{{ $district->id }}"
                                        {{ $district->id == $posts->district_id ? 'selected' : '' }}>
                                        {{ $district->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select @error('subdistrictsId') is-invalid @enderror" id="postSubDistrict"
                                ...
                                @foreach ($subdistricts as $subdistrict)
                                    <option value="{{ $subdistrict->id }}"
                                        {{ $subdistrict->id == $posts->subdistrict_id ? 'selected' : '' }}>
                                        {{ $subdistrict->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="d-flex gap-4">
                                <div>
                                    ...
                                    <input type="radio" name="lead" value="yes"
                                        {{ $posts->lead == 'yes' ? 'checked' : '' }}>
                                </div>
                                <div>
                                    <span>No</span>
                                    <input type="radio" name="lead" value="no"
                                        {{ $posts->lead == 'no' ? 'checked' : '' }}>
                                </div>
                            </div>
                            @error('lead')
                                <span class="text-danger">{{ $message }}</span>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="editor form-control @error('content') is-invalid @enderror" id="product_description" name="content"
                            rows="3" placeholder="Enter content">{!! $posts->description !!}</textarea>
                        @error('content')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
